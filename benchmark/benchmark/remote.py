...
class Bench:
    ...
    def set_tc_filter(self):
        asyncio.get_event_loop().run_until_complete(self._set_tc_filter())
    ...
    async def _reset_tc_filter(self):
        hosts = self.manager.hosts(flat=True)
        hosts_and_connections = await self._try_connect_all(hosts)
        hosts_to_connections = { h: c for h, c in hosts_and_connections }
        ...
        tasks = [ self._filter_one(h, c, tc_filter_cmd) for h, c in hosts_to_connections.items() ]
        await self._gather_and_parse(tasks, 'Reset')

    def reset_tc_filter(self):
       asyncio.get_event_loop().run_until_complete(self._reset_tc_filter())
    ...
    async def _run_clients(self, rate, committee, bench_parameters, connections):
    ...
    async def _run_primaries(self, committee, connections, faults, debug=False):
        ...
        tasks = []
        print(committee.primary_addresses())
    ...
    async def _run_single(
        ...
        if run_clients:
            ...
            workers_addresses = await self._run_clients(
                rate, committee, bench_parameters, hosts_to_connections)
                ...
                for i in range(bench_parameters.runs):
                    ...
                    try:
                        await self._run_single(
                            r, committee_copy, bench_parameters, hosts_to_connections, debug, consensus_only, run_clients
                        )

                        faults = bench_parameters.faults
                        await self._download_logs(consensus_only, committee=committee)
                        Print.info('Parsing logs and computing performance...')
                        logger = LogParser.process(PathMaker.logs_path(), consensus_only=consensus_only)
    ...
    def run(self, bench_parameters_dict, node_parameters_dict, debug=False, consensus_only=False, update=True, run_clients=False):
    ...
    async def try_run_clients(self, bench_parameters, committee, hosts):
        hosts_and_connections = await self._try_connect_all(hosts)
        hosts_to_connections = { host: connection for host, connection in hosts_and_connections }
        rate = bench_parameters.rate[0]
        await self._run_clients(rate, committee, bench_parameters, hosts_to_connections)
        

    def run_clients(self, bench_parameters_dict, debug=False, consensus_only=False, update=False):
        try:
            bench_parameters = BenchParameters(bench_parameters_dict)
        except ConfigError as e:
            raise BenchError('Invalid nodes or bench parameters', e)
        ...
        committee = Committee.from_file(PathMaker.committee_file())
        asyncio.get_event_loop().run_until_complete(
            self.try_run_clients(bench_parameters, committee, selected_hosts)
            )