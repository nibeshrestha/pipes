use super::*;
use crate::common::{
    block_from, chain, committee, committee_with_base_port, keys, listener, proposal,
    proposal_from, proposal_from_block, qc_from,
};
...
async fn generate_proposal() {
    ...
    let (tx_core_2, mut rx_proposer_2, _rx_commit_2) = core(l3, l3_key, committee(), store_path_2);

    // Send the first block to both leaders.
    tx_core_1
        .send(ConsensusMessage::Propose(ProposalMessage::Normal(
            p1.clone(),
        )))
        .await
        .unwrap();
    tx_core_2
        .send(ConsensusMessage::Propose(ProposalMessage::Normal(p1)))
        .await
        .unwrap();
    ...
    tx_core_2
        .send(ConsensusMessage::Propose(ProposalMessage::Normal(p2)))
        .await
        .unwrap();
